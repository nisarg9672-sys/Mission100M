...
export const getYahooPrice = async (ticker = 'BTC-USD') => {
  try {
...
export const getHistoricalData = async (ticker = 'BTC-USD', period = '1mo') => {
  try {
    const validatedTicker = validateTicker(ticker);
...
function validateTicker(ticker) {
  ...
  // Handle common Bitcoin variations
  const upperTicker = ticker.toUpperCase();
  if (upperTicker === 'BTC' || upperTicker === 'BITCOIN') {
    return 'BTC-USD';
  }
  
  if (upperTicker === 'BTC-USD' || upperTicker === 'BTCUSD') {
    return 'BTC-USD';
  }
