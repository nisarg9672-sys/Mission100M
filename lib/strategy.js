// lib/strategy.js - Enhanced trading strategy with more lenient safety controls
class TradingStrategy {
  constructor() {
    this.MIN_PRICE_CHANGE_PCT = 2; // Minimum 2% price movement to consider trades
    this.MAX_POSITION_SIZE = 0.05; // Maximum 0.05 ETH per trade (increased from 0.02)
    this.PROFIT_TARGET_PCT = 4; // Take profit at 4%
    this.STOP_LOSS_PCT = 2.5; // Stop loss at 2.5%
    this.RSI_OVERSOLD = 40; // More lenient RSI levels (changed from 35)
    this.RSI_OVERBOUGHT = 60; // More lenient RSI levels (changed from 65)
  ...
  shouldBuy(rsi, sma20, currentPrice, trend, bullishSignals, bearishSignals) {
    // More lenient buy conditions - need 3 out of 5 (changed from ALL 5)
    const conditions = [
      rsi < this.RSI_OVERSOLD,           // RSI oversold (now < 40)
    ...
    return trueConditions >= 3; // Need 3 out of 5 conditions (more lenient)
  }

  shouldSellOnTechnicals(rsi, sma20, currentPrice, trend, bearishSignals) {
    // Technical sell conditions - need at least 2
    const conditions = [
      rsi > this.RSI_OVERBOUGHT,        // RSI overbought (now > 60)
  ...
  calculatePositionSize(currentPrice = 3000) {
    // More aggressive position sizing (increased limits)
    if (currentPrice > 4000) return 0.02;      // $80+ per trade (increased from 0.01)
    if (currentPrice > 3000) return 0.03;     // $90+ per trade (increased from 0.015)  
    if (currentPrice > 2000) return 0.04;     // $80+ per trade (increased from 0.02)
    return this.MAX_POSITION_SIZE;            // Default max (0.05 ETH)
  }
}
