...
class AlpacaService {
  ...
  async getPosition(symbol) {
    try {
      ...
      // 404 error means no position exists - this is normal, not an error
      if (error.response && error.response.status === 404) {
        console.log(`No position found for ${symbol} (this is normal if you haven't opened a position yet)`);
        return null;
      }
